...
export class SqliteDatabase {
  ...
  static async initialize(): Promise<void> {
    try {
      const dbPath = path.join(process.cwd(), 'bhamail.db');
      ...
      logger.info('SQLite database connection established at:', dbPath);
    } catch (error) {
  ...
  static seedInitialData(): void {
    ...
    // Get user ID
    const user = db.prepare('SELECT id FROM users WHERE email = ?').get('demo@bhamail.com') as any;
    ...
    if (user) {
      ...
      // Get inbox folder
      const inbox = db.prepare('SELECT id FROM folders WHERE user_id = ? AND type = ?').get(user.id, 'inbox') as any;
      ...
      if (inbox) {
        ...
        createEmail.run(
          ...
          'demo@bhamail.com', 
          'Welcome to BhaMail! 🎉', 
          'Thank you for trying BhaMail. This is your local development environment with SQLite database. All features are now working without Docker!',
          false
        );
        ...
        createEmail.run(
          ...
          'demo@bhamail.com', 
          '[GitHub] Your repository BhaMail has been updated', 
          'New commits have been pushed to your repository. The latest changes include SQLite integration for local development.',
          false
        );
        ...
        createEmail.run(
          ...
          'Latest updates in web development, AI, and open source projects. This week: SQLite alternatives to Docker, local development tips, and more.',
          true
        );
      }
    }
    
    logger.info('Initial data seeded to SQLite database');
  }
}